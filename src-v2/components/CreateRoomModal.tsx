...
const CreateRoomModal: React.FC<CreateRoomModalProps> = ({ isOpen, onClose, onCreated }) => {
  ...
  const zikrArabic = zikrChoice === '__custom__' ? '' : getZikrDisplayInfo(zikrChoice).arabicText;
  ...
  const handleCreate = async () => {
    ...
    if (!zikrName) {
      setLocalError('Please choose or enter what will be read.');
      return;
    }
    const targetNum = parseInt(target, 10);
    if (isNaN(targetNum) || targetNum < 1 || targetNum > 100000000) {
      setLocalError('Enter a target between 1 and 100,000,000.');
      return;
    }
    const { startsAt, endsAt } = computeWindow();
    if (endsAt.getTime() <= Date.now()) {
      setLocalError('The end of the time window must be in the future.');
      return;
    }
  ...
  const copy = async (kind: 'code' | 'link', value: string) => {
    try {
      ...
      setLocalError('Could not copy — please copy it manually.');
    }
  };
      ...
      >
        {step === 'details' && (
          <>
            <div className="flex items-center justify-between mb-2">
              <h2 className="font-headline-lg text-headline-lg text-primary">New Shared Goal</h2>
              <button
                onClick={onClose}
            ...
            <p className="font-caption text-caption text-on-surface-variant mb-4">
              Read together, anywhere. Everyone sees the combined progress.
            </p>
            ...
            <div className="flex flex-col gap-6">
              ...
              <InputField
                label="Goal title"
                placeholder={`e.g., ${zikrName || 'SubhanAllah'} together`}
                value={title}
              ...
              <div className="flex flex-col gap-2">
                ...
                {zikrChoice === '__custom__' && (
                  <input
                    ...
                    placeholder="Enter the zikr / surah / intention"
                    maxLength={80}
              ...
              <InputField
                label="Combined target (all members together)"
                type="number"
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2 bg-surface-container-low p-1 rounded-xl">
                  {(['today', 'week', 'custom'] as PresetKey[]).map((key) => (
                    ...
                    >
                      {key === 'today' ? 'Today' : key === 'week' ? 'This week' : 'Custom'}
                    </button>
                  ))}
                </div>
                {preset === 'custom' && (
        ...
        {step === 'share' && created && (
          <div className="flex flex-col items-center text-center">
            ...
            <h2 className="font-headline-lg text-headline-lg text-primary mb-1">Room created</h2>
